    ...
    render(){
        return (
            <div>
                {/* <Link to='/Details/Details'> */}
                {
                    this.state.data.map((item)=>(
                        <div style={{border:'1px solid black',width:800,height:50,overflow:'hidden',background:'#999',marginTop:2,}} key={item.id}>
                            ...
                            <h4 style={{float:'left',marginTop:15,marginLeft:10}}>{item.reply_count}</h4><h2 style={{float:'left',marginTop:10}}>/</h2><h5 style={{float:'left',marginTop:20,}}>{item.visit_count}</h5>
                ...
                {/* </Link> */}
                {/* <Route exact path='/Details/Details/:id' component={Details}/> */}
            </div>
        )
    }
}
